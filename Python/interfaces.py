...
class IDChipController(I2CController):
    ...
    def get_manufacturer_code(self, addr_pins):
        return self.read(addr_pins, word_address=self.parameters['MANUFACTURER_CODE'].address)[0]

    # Method to get the device code. Should be 0x41.
    def get_device_code(self, addr_pins):
        return self.read(addr_pins, word_address=self.parameters['DEVICE_CODE'].address)[0]
...
class I2CDACController(I2CController):
    ...
    def write(self, addr_pins, data, register_name='DAC_DATA'):
        dev_addr = self.parameters['ADDRESS_HEADER'] + (addr_pins*2)
        ...
        # Compare with current data to mask unchanged values
        read_out = self.read(addr_pins, register_name)
        if read_out == None:
            print('Read for masking FAILED')
            return False
        current_data = 0
    ...
    def read(self, addr_pins, register_name='DAC_DATA'):
        ...
        if read_back_list == None:
            return None
        # Reading back data backwards because last byte in the list is the Most Significant Byte of the data.
        for byte in read_back_list[::-1]:
            read_back_data <<= 8
            read_back_data |= byte
    ...
    def write_voltage(self, addr_pins, voltage):
        self.write(addr_pins, voltage, 'DAC_DATA')

    # Method to enable the internal reference. Necessary to set the gain value.
    def enable_internal_reference(self, addr_pins):
        self.write(addr_pins, 0x1, 'REF_EN')
    ...
    def set_gain(self, addr_pins, gain):
        gain_dict = {
            1.5: 0b00,
            2: 0b01,
            3: 0b10,
        ...
        if gain_code == None:
            print('Invalid gain')
            return False
        self.write(addr_pins, gain_code, 'DAC_SPAN')

    # Method to get the gain value.
    def get_gain(self, addr_pins):
        gain_dict = {
            0b00: 1.5,
            0b01: 2,
            0b10: 3,
        ...
        gain_code = self.read(addr_pins, 'DAC_SPAN')
        return gain_dict.get(gain_code)

    # Method to configure the function generator.
    def config_func(self, addr_pins, function_name):  # TODO: figure out the input variables
        func_dict = {
        ...
        if func_code == None:
            ...
            return False
        self.write(addr_pins, func_code, 'FUNC_CONFIG')
    ...
    def start_func(self, addr_pins):
        self.write(addr_pins, 0b1, 'START_FUNC_GEN')

    # Method to stop function generation.
    def stop_func(self, addr_pins):
        self.write(addr_pins, 0b0, 'START_FUNC_GEN')

    # Method to configure the margins.
    def config_margins(self, addr_pins, margin_high=None, margin_low=None):
        if margin_high != None:
            self.write(addr_pins, margin_high, 'MARGIN_HIGH')

        if margin_low != None:
            self.write(addr_pins, margin_low, 'MARGIN_LOW')

    # Method to configure the number of bits to step through.
    def config_step(self, addr_pins, step):
        step_dict = {
            1: 0b000,
            2: 0b001,
            3: 0b010,
        ...
        if step_code == None:
            print('Invalid step')
            return False
        self.write(addr_pins, step_code, 'CODE_STEP')

    # Method to configuer the rate to step through each bit.
    def config_rate(self, addr_pins, rate):
        ...
        if rate_code == None:
            print('Invalid rate')
            return False
        self.write(addr_pins, rate_code, 'SLEW_RATE')

    # Method to reset the device using a software reset.
    def reset(self, addr_pins):
        self.write(addr_pins, 0b1010, 'SW_RESET')

    # Method to lock the registers of the device.
    def lock(self, addr_pins):
        self.write(addr_pins, 0b1, 'DEVICE_LOCK')

    # Method to unlock the registers of the device.
    def unlock(self, addr_pins):
        self.write(addr_pins, 0b0101, 'DEVICE_UNLOCK_CODE')

    # Method to power up the DAC.
    def power_up(self, addr_pins):
        self.write(addr_pins, 0b00, 'DAC_PDN')

    # Method to power down the DAC to 10K.
    def power_down_10k(self, addr_pins):
        self.write(addr_pins, 0b01, 'DAC_PDN')

    # Method to power down the DAC to high impedance (default).
    def power_down_high_impedance(self, addr_pins):
        self.write(addr_pins, 0b10, 'DAC_PDN')
...
# TODO: move count_bytes() to utils.py
def count_bytes(num): #count number of bytes
    bytes = 0
